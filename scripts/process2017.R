...
x2017_data <- x2017_data[,c(1,2,3,6,4,5,8)]


x2017_data$`Ratio to State Median Num` <- x2017_data$`Median Household Income.x` / merge$`Median Household Income.y`
x2017_data$`Ratio to State Median MOE` <- sqrt((merge$`Margins of Error.x`)^2 + ((merge$`Median Household Income.x`/merge$`Median Household Income.y`)*((merge$`Margins of Error.y`)^2))) / (merge$`Median Household Income.x`)
